   ...
   1. 정적 팩토리 메서드로 빈 초기화하기
      ...
      public class FixedDepositDaoFactory {
         ...
         public static FixedDepositDao getFixedDepositDao(String daoType, ...) {
            ...
            // daoType 의 인수값에 따라 FixedDepositHibernateDao, FixedDeppositIbatisDao, FixedDepositJdbcDao 클래스의 인스턴스 중 하나를 만들어내는 getFixedDepositDao 정적메서드를 정의함
      ...
      <constructor-arg index="0" value="jdbc"/> // constructor-arg : 스프링의 beans 스키마에서 정의, 생성자에게 인수를 넘기거나 정적 팩터리 메소드 또는 인스턴스 팩토리 메서드에 인수를 넘길 때 사용. index 속성값이 0이면 <constructor-arg>엘리먼트가 getfixedDepositDao 팩토리 메서드의 첫 번째 인수(타입은 daoType)를 정의한다. 
      ....
      </bean>
      ```
